...
namespace Bap.EventChannel
{
    ...
    public abstract class GeneralEventChannelSOEditor<T> : UnityEditor.Editor
    {
        // Reference to the original event channel (to set up button callback)
        private EventChannelSO<T> m_EventChannel;
        ...
        private void OnEnable()
        {
            if (m_EventChannel == null)
                m_EventChannel = target as EventChannelSO<T>;
        }
        ...
        public override VisualElement CreateInspectorGUI()
        {
            ...
            m_RaiseEventButton.RegisterCallback<ClickEvent>(evt => m_EventChannel.RaiseEvent(default));
